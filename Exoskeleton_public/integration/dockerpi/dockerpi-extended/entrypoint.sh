...
extra_hostfwd="${DOCKERPI_EXTRA_HOSTFWD:-}"
extra_append="${DOCKERPI_EXTRA_APPEND:-systemd.mask=boot.mount systemd.mask=serial-getty@ttyAMA0.service systemd.mask=hciuart.service}"
...
if [ "$target" = "pi1" ]; then
  ...
  memory=256m
  root=/dev/sda2
  nic_base="--net nic --net user,hostfwd=tcp::5022-:22"
  if [ -n "$extra_hostfwd" ]; then
    nic="${nic_base},${extra_hostfwd}"
  else
    nic="$nic_base"
  fi
  ...
  netdev_base="user,id=net0,hostfwd=tcp::5022-:22"
  if [ -n "$extra_hostfwd" ]; then
    netdev="${netdev_base},${extra_hostfwd}"
  else
    netdev="$netdev_base"
  fi
  nic="-netdev ${netdev} -device usb-net,netdev=net0"
...
exec ${emulator} \
  ...
  --dtb "${dtb}" \
  --kernel "${kernel}" \
  --append "rw earlyprintk loglevel=8 console=ttyAMA0,115200 dwc_otg.lpm_enable=0 root=${root} rootwait panic=1 ${append} ${extra_append}" \
  --no-reboot \
  --display none \
  --serial mon:stdio
