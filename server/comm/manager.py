"""ConnectionManager — manages TCP connections from multiple Pi agents.

Extended from Exoskeleton_public with:
- Telemetry channel (port 8613)
- Device manifest tracking per client
- Sim mode tracking
"""
...
class ClientSession:
    ...
    telemetry_conn: socket.socket | None = None
    telemetry_addr: tuple[str, int] | None = None
    device_manifest: list[dict[str, Any]] = field(default_factory=list)
    sim_mode: bool = False
...
class ConnectionManager:
    def __init__(
        ...
        self._telemetry_listener: socket.socket | None = None
        ...
        # Callbacks for persistence (set by app.py)
        self.on_telemetry: Any = None
        self.on_command_event: Any = None
    ...
    def ensure_started(self) -> None:
        with self._state_lock:
            ...
            self._telemetry_listener = self._create_listener(self.host, self.telemetry_port)
            self._running = True
        ...
        threading.Thread(target=self._accept_telemetry_loop, daemon=True).start()
        threading.Thread(target=self._heartbeat_monitor_loop, daemon=True).start()
        print(
            f"ConnectionManager listening on control={self.control_port}, "
            f"video={self.video_port}, telemetry={self.telemetry_port}"
        )
    ...
    def stop(self) -> None:
        with self._state_lock:
            ...
            telemetry_listener = self._telemetry_listener
        ...
        self._close_socket(telemetry_listener)
    ...
    def get_latest_telemetry(self, client_id: str) -> dict[str, Any] | None:
        with self._state_lock:
            return self._latest_telemetry.get(client_id)
                ...
                clients.append(
                    {
                        ...
                        "telemetry_connected": session.telemetry_conn is not None,
                        "device_manifest": session.device_manifest,
    ...
    def send_command(self, client_id: str, command: str) -> CommandEvent:
        ...
        if self.on_command_event:
            self.on_command_event(event.snapshot())
    ...
    def _initialize_control_connection(
        ...
        try:
            ...
            session = ClientSession(
                ...
                connected_at=now,
                last_heartbeat=now,
                device_manifest=payload.get("devices", []),
                sim_mode=payload.get("sim_mode", False),
            )
            ...
            print(f"Control client connected: {client_id} session={session_id} devices={len(session.device_manifest)}")
    ...
    def _video_reader_loop(
        self, client_id: str, session_id: str, conn: socket.socket
    ) -> None:
        try:
            while not self._stop_event.is_set():
    ...
    def _accept_telemetry_loop(self) -> None:
        while not self._stop_event.is_set():
            listener = self._telemetry_listener
            if listener is None:
                return
            try:
                conn, addr = listener.accept()
                ...
                continue
            threading.Thread(
                target=self._initialize_telemetry_connection,
                args=(conn, addr),
                daemon=True,
            ).start()

    def _initialize_telemetry_connection(
        self, conn: socket.socket, addr: tuple[str, int]
    ) -> None:
        client_id = ""
        session_id = ""
        try:
            ...
            if registration.get("type") != "telemetry_hello":
                raise ProtocolError("First telemetry message must be type=telemetry_hello")

            client_id = str(registration.get("client_id", "")).strip()
            session_id = str(registration.get("session_id", "")).strip()
            if not client_id or not session_id:
                raise ProtocolError("telemetry_hello missing client_id/session_id")

            with self._state_lock:
                session = self._sessions.get(client_id)
                if session is None or session.session_id != session_id:
                    raise ClientNotConnectedError(
                        f"Telemetry session mismatch for client {client_id}"
                    )
                old_conn = session.telemetry_conn
                session.telemetry_conn = conn
                session.telemetry_addr = addr

            self._close_socket(old_conn)
            conn.settimeout(None)
            print(f"Telemetry client connected: {client_id}")
            self._telemetry_reader_loop(client_id, session_id, conn)
        except Exception as err:
            print(f"Telemetry connection init failed: {err}")
            self._close_socket(conn)
            if client_id and session_id:
                with self._state_lock:
                    session = self._sessions.get(client_id)
                    if (
                        session
                        and session.session_id == session_id
                        and session.telemetry_conn is conn
                    ):
                        session.telemetry_conn = None
                        session.telemetry_addr = None

    def _telemetry_reader_loop(
        self, client_id: str, session_id: str, conn: socket.socket
    ) -> None:
        try:
            while not self._stop_event.is_set():
                ...
                if payload_size <= 0 or payload_size > self.max_control_line_bytes:
                    raise ProtocolError(f"Invalid telemetry payload size {payload_size}")
                ...
                with self._state_lock:
                    ...
                    if session is None or session.session_id != session_id:
                        break
                    self._latest_telemetry[client_id] = data

                if self.on_telemetry:
                    self.on_telemetry(client_id, data)

        except Exception as err:
            print(f"Telemetry reader loop ended for {client_id}: {err}")
        finally:
            self._close_socket(conn)
            with self._state_lock:
                session = self._sessions.get(client_id)
